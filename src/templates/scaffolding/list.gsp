    ...
    <table class="table table-bordered table-striped">
        ...
        <g:each in="\${${propertyName}List}" status="i" var="${propertyName}">
            <tr class="\${(i % 2) == 0 ? 'even' : 'odd'}">
                ...
                <td><g:formatDate date="\${${propertyName}.${p.name}}" format="dd MMMM yyyy hh:mm:ss"/></td>
                <% } else { %>
                <td>\${fieldValue(bean: ${propertyName}, field: "${p.name}")}</td>
                <% }
                }
                } %>
                <td><g:link controller="fileManager" action="renderFile"
                            id="\${${propertyName}.id}">download</g:link></td>
            </tr>
        </g:each>
        </tbody>
    </table>
